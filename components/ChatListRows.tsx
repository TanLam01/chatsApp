...
import { ChatMembers, chatMembersCollectionGroupsRef } from "@/lib/converters/ChatMembers";
...
function ChatListRows({ initialChats }: { initialChats: ChatMembers[] }) {
    ...
    const [members, loading, error] = useCollectionData<ChatMembers>(
        session && chatMembersCollectionGroupsRef(session?.user.id!),
        {
    ...
    if (members?.length === 0)
        return (
            <div className="flex flex-col justify-center items-center pt-40 space-y-2">
                ...
                <h2 className="pb-10">
                    Lets get your started by creating your first chat!
                </h2>
